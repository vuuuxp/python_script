...
mysql --host mysqlmaster -uroot -p$MYSQL_MASTER_PASSWORD -AN -e "GRANT ALL PRIVILEGES ON database_name.* TO'$MYSQL_REPLICATION_USER'@'localhost';"
mysql --host mysqlmaster -uroot -p$MYSQL_MASTER_PASSWORD -AN -e "CREATE USER '$MYSQL_HEARTBEAT_USER'@'%' IDENTIFIED BY '$MYSQL_HEARTBEAT_PASSWORD';"
mysql --host mysqlmaster -uroot -p$MYSQL_MASTER_PASSWORD -AN -e "GRANT ALL PRIVILEGES ON database_name.* TO'$MYSQL_HEARTBEAT_USER'@'localhost';"
